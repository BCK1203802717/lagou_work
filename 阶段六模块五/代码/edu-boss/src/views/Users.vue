  ...
  <section class="users">
    ...
    <el-dialog title="分配角色" :visible.sync="allocDialogVisible" width="30%">
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="handleAllocDialogConfirm(allocRoleIds)" size="small">确 定</el-button>
      </span>
    </el-dialog>
  </section>
</template>
...
export default {
  ...
  methods: {
    ...
    handleToggleStatus(item) {
      //const params={id:item.id,status:item.status}
    ...
    handleAllocDialogConfirm(allocRoleIds){
      const params={
        userId:this.allocAdminId,
        roleIdList:allocRoleIds
      }
      axios
    ...
    handleSelectRole(index, row) {
      ...
      axios
      .get("/user/findUserRoleById",{
        params:{
          id:row.id
        }
      })
      .then(res=>{
          this.allocRoleIds.length=0;
          res.data.content.map((item)=>{
            this.allocRoleIds.push(item.id);
          })
